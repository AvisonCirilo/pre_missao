...
class _AbaPainelState extends State<AbaPainel> {
  ...
  final List<String> _nomesEtapasRapazes = [
    "Aceitou o Desafio (Entrevistado)", "Ensino Médio Concluído", "Alistamento Militar",
    "Possui Mentor", "Metas com o Bispo", "Exame Médico", "Exame Odontológico", "Chamado Aberto no Sistema"
  ];
  final List<String> _nomesEtapasMocas = [
    "Aceitou o Desafio (Entrevistado)", "Ensino Médio Concluído", "Possui Mentor",
    "Metas com o Bispo", "Exame Médico", "Exame Odontológico", "Chamado Aberto no Sistema"
  ];

  final List<Map<String, dynamic>> _jovensPreparacao = [
    {'id': '1', 'nome': 'João Silva', 'idade': 19, 'sexo': 'Masculino', 'telefone': '5591900000000', 'etapas': <bool>[true, true, true, false, true, false, false, false]},
    {'id': '2', 'nome': 'Ana Beatriz', 'idade': 18, 'sexo': 'Feminino', 'telefone': '5591900000000', 'etapas': <bool>[true, false, false, false, false, false, false]},
  ];
                      ...
                      onPressed: () {
                        ...
                        setState(() {
                          if (isEdicao) {
                            ...
                            _jovensPerspectiva.add({'id': DateTime.now().millisecondsSinceEpoch.toString(), 'nome': nomeCtrl.text.trim(), 'idade': int.tryParse(idadeCtrl.text) ?? 0, 'sexo': sexoSelecionado, 'telefone': telefoneCtrl.text.trim()});
                          }
                        });
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        return CheckboxListTile(
                          title: Text(
                            listaEtapas[index],
                            style: TextStyle(fontWeight: etapasTemp[index] ? FontWeight.normal : FontWeight.bold, decoration: etapasTemp[index] ? TextDecoration.lineThrough : null, color: etapasTemp[index] ? Colors.grey : (isEscuro ? Colors.white : Colors.black87)),
                          ),
                          value: etapasTemp[index], activeColor: Colors.blue, checkColor: Colors.white,
                          side: BorderSide(color: isEscuro ? Colors.grey.shade400 : Colors.grey.shade700),
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: ElevatedButton.icon(
                          onPressed: () {
                            setState(() { jovem['etapas'] = List<bool>.from(etapasTemp); });
                            Navigator.pop(context); 

                            if (_calcularProgresso(jovem['etapas'], jovem['sexo']) == 1.0) {
                              Future.delayed(const Duration(milliseconds: 300), () {
                                setState(() { _jovensPreparacao.removeWhere((item) => item['id'] == jovem['id']); _qtdEnviados++; });
                              });
                            }
                          },
  ...
  void _iniciarPreparacao(Map<String, dynamic> jovem) {
    setState(() {
      ...
      _jovensPreparacao.add({'id': jovem['id'], 'nome': jovem['nome'], 'idade': jovem['idade'], 'sexo': jovem['sexo'], 'telefone': jovem['telefone'] ?? '', 'etapas': List.generate(totalEtapas, (index) => false)});
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Visão Geral da Ala', style: TextStyle(fontWeight: FontWeight.bold, color: corTexto)),
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Row(
                children: [
                  _construirCartaoDashboard("Perspectiva", _jovensPerspectiva.length.toString(), Colors.orange, Icons.radar, isEscuro),
                  const SizedBox(width: 10),
                  _construirCartaoDashboard("Preparação", _jovensPreparacao.length.toString(), Colors.blue, Icons.assignment_ind, isEscuro),
                  const SizedBox(width: 10),
                  _construirCartaoDashboard("Enviados", _qtdEnviados.toString(), Colors.green, Icons.check_circle, isEscuro),
                ],
              ),
            ),
            SizedBox(
              ...
              child: ListView(
                ...
                children: ['Todos', 'Exame Médico', 'Exame Odontológico', 'Alistamento Militar', 'Metas com o Bispo'].map((f) => _construirFiltro(f, isEscuro)).toList(),
              ),
            ),
            const SizedBox(height: 20),
            ...
            if (listaFiltrada.isEmpty)
              Padding(padding: const EdgeInsets.all(16.0), child: Text(_filtroAtual == 'Todos' ? "Nenhum jovem em preparação." : "Nenhum jovem pendente de $_filtroAtual.", style: const TextStyle(color: Colors.grey)))
            else
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12), side: BorderSide(color: isEscuro ? Colors.white12 : Colors.grey.shade200)), elevation: 0,
                    child: InkWell( 
                        ...
                        child: Row(
                          children: [
                                ...
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: Row(
                                      mainAxisSize: MainAxisSize.min,
                                      children: [
                                        Icon(statusAtual['icone'], size: 12, color: statusAtual['cor']), const SizedBox(width: 4),
                            ...
                            Stack(
                              alignment: Alignment.center,
                              children: [
                                SizedBox(height: 45, width: 45, child: CircularProgressIndicator(value: progressoAtual, backgroundColor: isEscuro ? Colors.black26 : Colors.grey.shade200, color: statusAtual['cor'], strokeWidth: 4)),
                                Text("${(progressoAtual * 100).toInt()}%", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: corTexto)),
                              ],
                            )
                          ],
                        ),
            ...
            if (_jovensPerspectiva.isEmpty)
              const Padding(padding: EdgeInsets.all(16.0), child: Text("Nenhum missionário em perspectiva.", style: TextStyle(color: Colors.grey)))
            else
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text("${jovem['idade']} anos", style: const TextStyle(color: Colors.grey)),
  ...
  Widget _construirFiltro(String titulo, bool isEscuro) {
    ...
    return Padding(
      ...
      child: FilterChip(
        label: Text(titulo, style: TextStyle(color: selecionado ? Colors.white : (isEscuro ? Colors.white70 : Colors.black87))),
        selected: selecionado, selectedColor: Colors.blue, backgroundColor: isEscuro ? const Color(0xFF1E1E1E) : Colors.white,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20), side: BorderSide(color: selecionado ? Colors.blue : (isEscuro ? Colors.white12 : Colors.grey.shade300))),
        onSelected: (bool value) => setState(() => _filtroAtual = titulo),
      ),
    );
  }
    ...
    return Expanded(
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isEscuro ? const Color(0xFF1E1E1E) : Colors.white, borderRadius: BorderRadius.circular(16),
