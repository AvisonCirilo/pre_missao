...
class _AbaDashboardEstacaState extends State<AbaDashboardEstaca> {
  // Simulando que o líder logado pertence à "Estaca Norte"
  final String _minhaEstaca = "Estaca Norte";

  // Banco de Dados Falso (já filtrado apenas para a Estaca Norte)
  final List<Map<String, dynamic>> _jovensEstaca = [
                    ...
                    child: Column(
                      children: [
                        ...
                        Expanded(
                          child: TabBarView(
                            children: [
                              // Checklist (Modo Leitura para a Estaca)
                              ListView.builder(
  ...
  Widget build(BuildContext context) {
    ...
    List<Map<String, dynamic>> jovensFiltrados = _jovensEstaca.where((jovem) {
      return jovem['nome'].toLowerCase().contains(_termoBusca.toLowerCase());
    }).toList();

    // Agrupa apenas por Unidade (pois a estaca já é a dele)
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Painel $_minhaEstaca', style: TextStyle(fontWeight: FontWeight.bold, color: corTexto)), 
        backgroundColor: corFundo, elevation: 1
      ),
      body: Column(
        children: [
          // DASHBOARD DE GARGALOS DA ESTACA
